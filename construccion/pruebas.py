...
with sync_playwright() as pw:
    ...
    # --- Llegar al paso 2 ---
    pg.evaluate("guarda('materia','residuos'); guarda('resp_tipo','desconocido'); irA(2)")
    ...
    for c in ['entre_calle1','entre_calle2','fachada','referencias']:
    ...
    forzado = pg.evaluate("""() => {
        guarda('mas_lugar','');
        const prev = {v: OBLIG.referencias.vigente, d: OBLIG.referencias.dgiva, validar: cfg.validar};
        OBLIG.referencias.vigente = true; OBLIG.referencias.dgiva = true;
        cfg.validar = true;
        guarda('referencias','');
        const paso = valida(2);
        const abierto = val('mas_lugar');
        OBLIG.referencias.vigente = prev.v; OBLIG.referencias.dgiva = prev.d; cfg.validar = prev.validar;
    ...
    p2.evaluate("guarda('materia','residuos'); guarda('resp_tipo','desconocido'); irA(2)")
