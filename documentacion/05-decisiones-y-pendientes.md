...
| DEC-07 | ~~Atender localmente las ocho ANP federales con convenio de coadministración~~ | 18 sep 2026 | Instrucción del área sustantiva. **Superada por DEC-89 (20 sep 2026)**: la cláusula SEGUNDA del convenio deja a salvo las facultades federales de inspección, de modo que el criterio vigente —provisional— turna a la PROFEPA. La cuestión está a consulta en P-22 |
...
| DEC-97 | **El panel de validación se simplifica: fuera la variante C de identificación y el conmutador de esquemas** | 20 sep 2026 | El panel guardaba decisiones que ya estaban tomadas y las presentaba como si siguieran abiertas. La **variante C —identificación obligatoria, sin anonimato—** contradecía a DEC-03, que admitió la denuncia anónima desde el primer día: quien abría el panel podía apagar la ruta anónima y no lo sabía. Se retira con su bandera `cfg.ident`, y una comprobación cuida que las tres rutas —Llave CDMX, datos escritos y anónima— se ofrezcan siempre, sin configuración que las cambie. El **conmutador entre «esquema DGIVA» y «formato vigente»** es de cuando el formulario se presentaba a una sola Dirección General y había que comparar dos propuestas de obligatoriedad; el formulario es de la Secretaría y tiene un solo conjunto de campos obligatorios. La bandera `dgiva` del catálogo pasa a llamarse **`oblig`** —el nombre viejo decía de quién era la propuesta, no qué gobierna— y `cfg.esquema` desaparece: `esObligatorio()` lee una sola bandera. La bandera `vigente` **se conserva como dato documental**, no de comportamiento: sigue registrando qué pedía la Ficha de Denuncia de 2016 para poder responder qué dato se dejó de exigir y cuál se agregó, y una comprobación verifica que forzarla no cambie nada. Además, la nota que explica el panel pasa **al principio** y se escribe a nombre de la Secretaría, no de la DGIVA; los dos escenarios que sólo servían para comparar esquemas se funden en uno; y los tres enlaces del final quedan juntos. El panel pasa de seis grupos y siete controles a una nota, tres interruptores, los escenarios y tres enlaces |
| DEC-98 | El documento 10 se genera del prototipo, como el 09 | 20 sep 2026 | El documento decía «Generado desde el prototipo» y no lo estaba: sus tablas se mantenían a mano, y por eso seguía documentando la variante C y los dos esquemas después de retirarse, y prometía competencia local en las Áreas Naturales Protegidas federales después de que el criterio cambió. **Es el mismo defecto que ya había mordido al documento 09** —un documento que afirma estar generado y no lo está miente con más autoridad que uno que no lo afirma—, y se cierra igual: `generar_doc10.py` lee `BIFURCACIONES` y `ESCENARIOS` del prototipo y conserva la parte redactada a mano del apartado 3 en adelante, deteniéndose si no la encuentra. Se retira del documento la columna del área que atiende cada escenario, que se repetía a mano y era justo por donde se separó: la resuelve el cruce espacial al cargar el escenario |
| DEC-99 | **Fuera la columna del formato de 2016; el límite de envíos deja de ofrecer el correo; el asterisco de obligatorio, en todos los campos** | 20 sep 2026 | Tres correcciones pedidas en la misma sesión. **La columna «Formato de 2016»** del mapeo se retira con su bandera `vigente`: DEC-97 la había conservado como dato documental y, vista en pantalla junto a la de obligatoriedad, invitaba a leerse como una segunda regla. Con ella se va el campo `responsables`, que sólo existía porque lo pedía aquel formato y que **no se rendía en ninguna pantalla**: estaba en el catálogo, en el mapeo y en la revisión, y no había modo de llenarlo. **El límite de envíos** ofrecía cuatro salidas, dos de ellas impropias: «enviarla por correo electrónico» abría una vía de recepción que nadie diseñó ni tiene quien la turne, y explicar el límite por la conexión compartida pedía a la persona entender la infraestructura. Quedan dos: volver a intentarlo por esta misma plataforma, o presentarla en la Oficialía de Partes. **La marca de obligatorio** la ponía cada constructor de campo por su cuenta, y los dos que no son `<input>` —las preguntas de sí o no y la hora— se habían quedado sin ella: «¿El lugar tiene calle y número?» es obligatoria y no lo decía. Ahora la llevan los cinco constructores, y una comprobación recorre los cinco pasos exigiendo, para cada campo obligatorio en ese momento, el asterisco, el «(obligatorio)» para lector de pantalla y un renglón donde mostrar su error. De paso, el resumen de errores dejó de arrastrar «* (obligatorio)» en cada renglón —un lector de pantalla decía «asterisco obligatorio» cuatro veces en una lista de cuatro— y su renglón «Punto en el mapa» dejó de enlazar a un ancla inexistente |
| DEC-100 | **Correcciones de la auditoría general del 20 de septiembre** | 20 sep 2026 | Se auditaron los quince documentos, el prototipo y la cadena de construcción. Lo corregido, por orden de lo que el ciudadano habría visto: **(1) La ficha del cruce se contradecía a sí misma en las ocho Áreas Naturales Protegidas federales con convenio.** Turnaba a la PROFEPA, lo advertía, y debajo imprimía —en verde, que es el color de lo confirmado— el texto de cuando la Secretaría atendía: «La denuncia se atiende en el ámbito local». Era lo último que se leía. Residuo de DEC-07, que DEC-89 revirtió. **Ninguna de las 343 comprobaciones lo detectó** porque todas medían el estado y ninguna leía la pantalla. **(2) Tres citas penales, y dos estaban mal.** El artículo 344 BIS se citaba en tala, terráceos y cascajo. Verificado contra el texto vigente (G.O. CDMX 24 dic 2025): el 344 BIS corresponde a terráceos —extracción de suelo o cubierta vegetal—; la tala es el **345 BIS** y el cascajo es el **344**. Se precisa además el ordenamiento: Código Penal **para el Distrito Federal**. **(3) El botón «Descartar» del borrador no hacía nada:** anclaba la confirmación en un contenedor del paso 6 y se pulsaba desde la portada. **(4) Los límites de archivos se anunciaban y no se aplicaban:** entraban archivos de cualquier tamaño y tipo, y los sobrantes se descartaban en silencio. **(5) La zona de carga decía «arrastra» y nadie recogía el arrastre:** soltar un archivo hacía que el navegador lo abriera y se perdiera la pantalla. **(6) El foco se perdía en cada cambio de paso**, de modo que quien navega con teclado tenía que tabular desde el principio del documento seis veces. **(7) El acuse prometía a nombre de la PROFEPA los plazos y la unidad de seguimiento de la Secretaría.** **(8) El folio decía DGIVA siempre**, incluso cuando el cruce turnaba a la DGCORENADR y el acuse lo decía dos párrafos más abajo. **(9) Lo capturado en una rama de responsable sobrevivía al cambio a otra:** dejaba de verse en pantalla, seguía en la revisión y habría viajado al expediente. **(10) Cinco de los veinte botones «Editar»** llevaban a campos que esa ruta no rinde. Más el paso 4 que no validaba al continuar mientras la barra de pasos sí; la pluralización de «Faltan 1 dato»; dos ramas que declaraban el paso 2 y viven en el 3; un escenario que abría donde no se ve lo que anuncia; el nombre de la Procuraduría mal escrito; y nueve reglas de estilo y dos ramas de código sin nada que las usara. Se agrega la **batería 14**, que mira lo que la persona ve y lo que la persona pulsa: 381 comprobaciones en catorce baterías |
| DEC-101 | **La dirección escrita coloca el punto sola; sin dirección, las otras dos vías** | 20 sep 2026 | Había un botón —«Buscar la dirección que escribiste»— que pedía a la persona hacer dos veces el mismo trabajo: escribir dónde ocurre y después pulsar algo para que el formulario lo usara. Se retira. **Al salir de cualquiera de los cuatro campos del domicilio, el formulario coloca el punto solo**, igual que ya ocurre al pegar un enlace de Google Maps (DEC-90). El disparo no se pide campo por campo: lo engancha `campo()` a quien pertenece al domicilio del lugar, como el aviso de formato de DEC-91; el código postal hace las dos cosas. **Tres cautelas, y las tres importan.** *Sólo si no hay punto todavía*: si la persona ya lo colocó o lo arrastró, corregir después la colonia no puede deshacerle el ajuste fino. *Sólo con calle y algo más* —colonia o código postal—: con la calle sola, en esta Ciudad, el resultado sería una moneda al aire. *Y no repite la misma consulta*, porque el disparo ocurre al salir de cada uno de los cuatro campos. El punto que pone la dirección **se puede mover y no se pide confirmarlo**: es una propuesta, y el cruce que decide la competencia se hace sobre el punto que quede. **En la ruta sin domicilio no hay dirección que buscar**, de modo que el bloque se enuncia distinto: ahí las vías son pegar la ubicación o dar un clic, y el texto ya no promete lo que en esa ruta no ocurre. De paso se retiró la alcaldía de la consulta: la escribe el cruce a partir del punto, así que usarla para buscar la dirección era alimentar la búsqueda con el punto anterior. Sigue en pie lo que P-12 advierte: el servicio público de Nominatim, al que el prototipo apunta, **prohíbe este uso**, y la versión funcional necesita el buscador propio |
| DEC-102 | **La búsqueda espera a que la dirección esté completa, y consulta una sola vez** | 20 sep 2026 | Corrige a DEC-101 el mismo día. Disparar al salir de cada campo producía **dos o tres consultas por denuncia capturada** —al escribir la colonia, otra al escribir el código postal—, y tenía un defecto peor que el volumen: **las primeras consultas iban con la dirección a medias**, de modo que el peor resultado era el primero que la persona veía y el punto se colocaba mal antes de colocarse bien. Ahora la búsqueda espera a que estén llenos **todos los campos exigibles del domicilio** y entonces consulta una vez. El número exterior es opcional: no se espera por él, pero si está se usa. La exigibilidad se lee del catálogo y **no de `esObligatorio()`**, que devuelve falso mientras la validación del panel esté apagada: la búsqueda no puede depender de un interruptor que existe para revisar el formulario. Corregir después la dirección sí vuelve a consultar, porque es otra dirección. Tres motivos, y el de la política de uso es el tercero: **la consulta es más precisa** con la dirección completa; **la persona ve un solo resultado** en vez de un punto que salta; y se reduce a una consulta por denuncia. Aun así **no basta para publicar contra el servicio público de Nominatim**, cuya política excluye el uso pesado y sistemático con independencia de cuántas consultas haga cada persona: sigue en pie lo que P-12 resuelve |
| DEC-103 | La ficha del cruce deja de repetir lo que ya dice arriba | 21 sep 2026 | Debajo de «Tipo de zona: Suelo urbano» y «Atiende: Dirección General de Inspección y Vigilancia Ambiental», la ficha añadía: «El punto se ubica en suelo urbano, materia que corresponde a esta dirección general». En los turnados locales la ficha se explica sola. La nota queda sólo donde hace falta explicar algo que los renglones no dicen: la competencia federal |
| DEC-104 | **Se verifican los fundamentos de las dieciocho materias contra el texto vigente: tres eran imputaciones equivocadas** | 21 sep 2026 | Se extrajo el texto de la Ley Ambiental de la Ciudad de México (última reforma 23 jul 2026), del Código Penal para el Distrito Federal, de la Ley de Residuos Sólidos del Distrito Federal y de las tres normas ambientales, y se cotejó artículo por artículo lo que el formulario muestra al ciudadano. **Tres citas eran de otra materia.** `arbolado` invocaba el **art. 106**, que obliga a las alcaldías a crear áreas verdes cuando no llegan a nueve metros cuadrados por habitante: nada que ver con podar un árbol. Pasa a los **arts. 108, 109 y 112**, que son el régimen de autorización, restitución y remisión al Código Penal. `impacto` invocaba el **art. 53**, que es la Manifestación Ambiental Única —el instrumento de fuentes fijas en operación—, cuando la materia es de obras. Pasa a los **arts. 25 y 50**. `descargas` invocaba el **art. 230**, que regula al particular que **sí cumple** los límites y al que la Secretaría puede fijarle condiciones más estrictas: fundar en él una denuncia invierte el sentido de la norma. Pasa a los **arts. 223, 226 y 227**, y el 226 es la prohibición: «Queda prohibido descargar aguas residuales sin tratamiento, en cualquier cuerpo o corriente de agua». **El art. 92 era un comodín**: aparecía en tres materias —cascajo, terráceos y otra afectación a zona protegida— y en ninguna dice nada sobre conducta sancionable; es una regla de planeación territorial. Se retira de las tres. **Y faltaba el artículo 123**, que es el puente sin el cual las prohibiciones del art. 138 —escritas para Áreas Naturales Protegidas— no alcanzan a las Áreas de Valor Ambiental: «Las prohibiciones que establece la presente Ley aplicables a las Áreas Naturales Protegidas, deberán observarse para las áreas de valor ambiental». Cuatro materias lo necesitaban, y son las que cubren la mayoría de las denuncias en barrancas y bosques urbanos. Se corrige además el nombre de la **Ley de Residuos Sólidos del Distrito Federal**, que el formulario llamaba «de la Ciudad de México», y las dos materias de residuos dejan de citar un ordenamiento sin señalar precepto. Quedan confirmadas las tres citas penales de DEC-100. En total, **quince de las dieciocho materias cambian su fundamento**; tres estaban bien: emisiones, quema y agroquímicos |
| DEC-105 | **Cinco correcciones del paso 2 y del paso 3** | 21 sep 2026 | **El nombre del lugar** ya no dice «el bosque, el parque»: esos los propone la capa cuando el punto cae dentro. Queda «el paraje, la comunidad, el canal, el camino», que es lo que nadie puede adivinar por el punto. **El selector de tipo de establecimiento** traía el control del sistema y se veía distinto del resto de campos —otra letra, otra altura, otra flecha—; se le quita esa apariencia, se le dibuja la flecha y «Selecciona…» va en gris hasta que se elige. Se le agrega la ayuda que faltaba —si ninguna opción encaja, elegir «Otro»— y sale «o chatarrería» de la opción de centro de acopio. **La persona señalada**: «¿Cómo se le identifica?» y «Dónde se le localiza» pedían una media filiación y un domicilio, con el tono de un parte policial y no el de una denuncia ambiental. El domicilio se retira —es dato personal de un tercero que no consintió nada— y la pregunta pasa a **«¿Qué relación tiene con el lugar?»**: si es el dueño, quien encargó la obra, el encargado del negocio. **La autoridad señalada** se preguntaba en un campo libre, que producía «la delegación», «agua», «el gobierno»: nada con lo que se pueda dirigir la recomendación del art. 331. Ahora se pregunta primero el **ámbito** —Gobierno de la Ciudad, alcaldía, federal, otra— y después el nombre sobre un catálogo dependiente **que acepta escribir la sigla**. Las dependencias de la Ciudad salen del **artículo 16 de la Ley Orgánica del Poder Ejecutivo y de la Administración Pública de la Ciudad de México**, texto vigente al 13 de julio de 2026; las alcaldías son las dieciséis; la lista federal es de apoyo y no sale de un ordenamiento local, por eso siempre se puede escribir otra |
| DEC-106 | **La descripción de «Emisiones a la atmósfera» deja de nombrar la competencia** | 21 sep 2026 | La tarjeta decía «un establecimiento industrial, mercantil o de servicios de jurisdicción local». «De jurisdicción local» es un criterio de reparto de competencia entre la Federación y la Ciudad, no algo que quien denuncia pueda reconocer al elegir la categoría; ese reparto lo resuelve después el cruce de capas y la ficha de la ruta. Queda «un establecimiento industrial o mercantil». **Observación abierta:** con el recorte la descripción ya no nombra los establecimientos de servicios —restaurantes, tintorerías, panaderías—, que son la fuente más frecuente de queja por olores, y las categorías hermanas de descargas y de residuos sí los nombran. Pendiente de confirmar si se restituye «o de servicios» |
| DEC-107 | **Las descripciones de aire, ruido y vehículos se recortan a lo que se ve** | 21 sep 2026 | Tres tarjetas del grupo «Aire, ruido y vehículos» cargaban texto que no ayuda a elegir la categoría. **Ruido** decía «Ruido, vibraciones, energía térmica o lumínica, olores o anuncios que rebasan los límites permisibles y provienen de un establecimiento» y queda «Ruido, vibraciones, olores, contaminación visual o lumínica». **Quema** pierde «Está prohibida salvo casos excepcionales con aviso previo a la Secretaría»: es el régimen jurídico del acto, no una seña para reconocerlo, y aparece más adelante en el detalle del fundamento. **Vehículo contaminante** pierde «incluidos los del transporte de carga y de pasajeros». El criterio queda fijado: la descripción de la tarjeta dice **qué se ve o se huele**; la competencia, el umbral y el régimen viven en el fundamento y en la ficha de la ruta. **Observación abierta:** el recorte de ruido deja fuera «energía térmica», que el artículo 214 sí enumera, y el de vehículos deja fuera el transporte concesionado, que es el origen más frecuente de la queja por humo |
| DEC-108 | **El paso 1 se ordena por lo que se denuncia, y el grupo se referencia por clave** | 22 sep 2026 | Los bloques y las materias dentro de cada bloque iban en el orden del catálogo legal. Medido en el artefacto, a 390 px, eso dejaba **«Impacto ambiental de una obra» —el 28.5 % de la base histórica— a 2,363 píxeles: la tercera pantalla**, detrás de «Verificentro irregular», que es el 0.1 %. Reordenados por el volumen observado en la base de la CIVASU (2,175 denuncias de 2024 a 2026, documento 17), queda **a 672 px, el primero que se ve**. La cobertura de lo visible sin desplazar sube del 21.9 % al 28.5 % de las denuncias, **a costa de una tarjeta menos a la vista**: un bloque de una sola tarjeta cuesta su encabezado, y eso se dice sin adornos. **Dos excepciones deliberadas al criterio de frecuencia**, anotadas en el código: el cajón de sastre va al final de su bloque pese a tener 307 registros —arriba se llevaría las denuncias que tienen supuesto propio—, y terráceos y agroquímicos se conservan pese a no tener ninguno, porque esa ausencia mide el canal y no la realidad (D-16). **De paso, el grupo dejó de referenciarse por índice de arreglo y pasó a clave con nombre**: con índices, reordenar los bloques desalinea en silencio las materias que los apuntan, que es exactamente la operación que abrió este bloque de cambios |
| DEC-109 | **Se incorpora «Plásticos de un solo uso» como materia, con el fundamento como hueco visible** | 22 sep 2026 | No estaba en el catálogo pese a ser la cuarta materia en volumen de la base histórica —202 registros, 9.3 %— y **la de mejor rendimiento de todas: cero por ciento de incompetencia en los tres años y 76.7 % de visita practicada**, la tasa más alta. El patrón es homogéneo: globos metalizados y bolsas en sucursales de cadena. **El fundamento queda como `[Art. __, fracc. __]` a la vista**, porque DEC-104 verificó los de las dieciocho materias existentes contra el texto vigente y éste no se ha verificado; escribirlo sin verificar es lo que la norma del proyecto prohíbe. **El icono es provisional**: se reutiliza el de residuos sólidos, de modo que la tarjeta queda con el mismo icono que la vecina «Residuos de un establecimiento». Es un defecto visible y pide un icono propio del set institucional |
| DEC-110 | Manzana y lote, dos campos opcionales propios en el paso 2 | 22 sep 2026 | En el **5.8 %** de los domicilios de la base histórica el predio no se identifica por número sino por manzana y lote. Hoy esa referencia acaba escrita dentro del campo de calle, de modo que **la dirección llega partida a la orden de visita**. Dos campos propios la conservan entera. Van después del número exterior y antes de la colonia, sólo en la ruta con domicilio, y ninguno es obligatorio |
| DEC-111 | `construir.py` resuelve sus rutas contra la raíz del repositorio | 22 sep 2026 | Tenía escritas fijas las rutas de origen y destino, y apuntaban a carpetas de la máquina donde se escribió el guion: **el comando que documenta el LEEME no corría desde el repositorio**. Es la regla de cero valores fijos, que los otros dos generadores ya cumplían. Se resuelven contra la raíz, con sustitución por variable de entorno |
| DEC-112 | Un bloque de una sola tarjeta ocupa el renglón entero | 22 sep 2026 | **Defecto encontrado mirando la captura, no midiendo.** Con el orden nuevo, el primer bloque tiene una sola materia, y la rejilla de dos columnas le dejaba **media fila vacía a su derecha**: un hueco que se lee como una opción que falta, y en lo primero que se ve de la pantalla. Excepción deliberada a la rejilla, anotada junto a la regla base. Aparece también al filtrar con el buscador hasta un solo resultado |
...
**Verificación de la opción D, 20 de septiembre de 2026.** Se revisó qué ofrece hoy el Gobierno de la Ciudad. **La Ciudad publica los datos pero no expone un geocodificador**: el Sistema Abierto de Información Geográfica y el Portal de Datos Abiertos ofrecen descarga de capas —catálogo de colonias, vialidades, división geográfica—, no un servicio de búsqueda por dirección. La opción D, tal como estaba planteada, no existe.

Eso no la cancela: la **convierte en insumo de la opción B o C**. Con el catálogo de colonias y la capa de vialidades de la Ciudad, más la Red Nacional de Caminos y el Marco Geoestadístico del INEGI —los cuatro de descarga libre—, se puede construir un buscador propio que resuelva **calle y colonia**, que es exactamente lo que este formulario necesita: acercar el mapa para que la persona coloque el punto, no dar la coordenada exacta de un domicilio. El número exterior no hace falta para eso.

*Cifras que conviene tener a la mano.* Photon es programa libre bajo licencia Apache 2.0 y publica volcados semanales por país, de modo que no hay que construir el índice; su instancia pública, igual que la de Nominatim, limita el uso y no sirve para producción. Los proveedores comerciales con nivel gratuito —LocationIQ con cinco mil consultas diarias, OpenCage con dos mil quinientas— alcanzarían por volumen, pero el autocompletado consume varias consultas por búsqueda y, sobre todo, **la dirección del domicilio de quien denuncia viajaría a un tercero**, lo que obliga a declararlo como transferencia en el aviso de privacidad.

*Recomendación actualizada.* **Solicitar formalmente a la ADIP el catálogo de colonias y la capa de vialidades**, que además resuelve P-16, y construir con ellos el buscador propio. Photon queda como respaldo si el dato de la Ciudad resulta insuficiente. **Lo que no debe hacerse es publicar el formulario apuntando al servicio público de Nominatim**, que es lo que el prototipo hace hoy y que su política prohíbe.
---
